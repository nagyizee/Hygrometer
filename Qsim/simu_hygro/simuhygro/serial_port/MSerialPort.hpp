...
//
// Step 1:
//  Make an instance of the MSerialPort class
//      MSerialPort *MySerialPort = new MSerialPort();
//
// Step 2:
...
//
// TODO:
//      Come up with a MSerialPort method to read a line at a time
//      given that a line ends in a line terminator like \n or \r
//
//////////////////////////////////////////////////////////////////////////

#ifndef MSERIALPORT_HPP
#define MSERIALPORT_HPP

// library linking info
#include "MSerialPort_Global.hpp"
...
public:
    ...
    ~MSerialPort();
    int openPort(QString *device_Name, int _buad, int _byteSize, int _stopBits, int _parity, uint8_t ev_char);
    void closePort();

    // data fetcher, get next byte from buffer
    uint8_t getNextByte();

    // returns the currently available number of bytes. Note !!!, may change between cals since it does serial port polling
    uint32_t bytesAvailable();
    // check if message queue has more event characters. Note !!! it does serial port polling
    bool checkForEventChar();
    ...
    int getBytes( uint8_t *buffer, uint8_t until_chr, int size );
...
class MSerialPort : public QThread
{
    Q_OBJECT
public:
